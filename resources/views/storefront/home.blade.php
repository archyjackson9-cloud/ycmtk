        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-24 relative z-10">
            @if($heroBanners->isNotEmpty())
                ...
                <div class="grid lg:grid-cols-2 gap-12 items-center">
                    <div class="space-y-6">
                        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full backdrop-blur-md bg-white/10 border border-white/20 text-emerald-300 text-xs font-semibold">
                            <span class="h-2 w-2 rounded-full bg-emerald-400 animate-pulse"></span>
                            <span>Direct Harvest • {{ config('cymarket.pilot_zone_name') }}</span>
                        </div>
                        ...
                        @if($banner->link_url)
                            <div class="pt-2">
                                <a href="{{ $banner->link_url }}" 
                                   class="inline-flex items-center gap-2 bg-gradient-to-r from-emerald-500 to-brand-600 hover:from-emerald-600 hover:to-brand-700 text-white font-bold px-8 py-3.5 rounded-full shadow-lg shadow-emerald-950/30 hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all">
                    ...
                    <div class="relative">
                        <div class="rounded-3xl overflow-hidden border border-white/20 shadow-2xl shadow-black/40">
                            @if($banner->hasVideo())
                                <video src="{{ $banner->videoUrl() }}" class="w-full h-full aspect-video object-cover" autoplay muted loop playsinline
                                       @if($banner->imageUrl()) poster="{{ $banner->imageUrl() }}" @endif>
                                </video>
                            @else
                                <img src="{{ $banner->imageUrl() }}" alt="{{ $banner->title }}" class="w-full h-auto object-cover">
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="grid lg:grid-cols-12 gap-12 items-center">
                    ...
                    <div class="lg:col-span-7 space-y-6 text-center lg:text-left">
                        ...
                        <h1 class="text-4xl sm:text-5xl lg:text-6xl font-black tracking-tight leading-[1.1] text-white">
                            Farm-Fresh Harvest, <br class="hidden sm:inline">
                            <span class="bg-gradient-to-r from-emerald-300 via-emerald-100 to-amber-200 bg-clip-text text-transparent">Delivered to Your Door.</span>
                        </h1>
                        
                        <p class="text-base sm:text-lg text-emerald-100/80 max-w-xl mx-auto lg:mx-0 font-normal leading-relaxed">
                            Every fruit, vegetable, and grain on CY-Market is harvested directly from our own certified farms. No third-party middlemen — only genuine farm-fresh goodness.
                        </p>
